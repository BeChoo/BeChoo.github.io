...
const Hotel = () => {
 ...
 const { checkIn, checkOut, adults } = JSON.parse(localStorage.getItem('hotelParams')) || {};
...
const fetchHotelOffers = async (hotelId, checkInDate, checkOutDate, adultsNumber) => {
    try {
  ...
  const toggleSave = async () => {
    ...
    const aboutThisProperty = hotel.propertyContentSectionGroups?.aboutThisProperty?.sections[0];
...
<section id="details">
                ...
                {
                    aboutThisProperty?.bodySubSections.find(subSection =>
                        subSection?.elements[0]?.header?.text === summary?.name
                    ) && (
                        <>
                            <h3 style={{ fontWeight: 'bold' }}>
                                {aboutThisProperty.bodySubSections.find(subSection => subSection?.elements[0]?.header?.text === summary?.name).elements[0].header.text}
                            </h3>
                            <p>
                                {aboutThisProperty.bodySubSections.find(subSection => subSection?.elements[0]?.header?.text === summary?.name).elements[0].items[0].content.text}
                            </p>
                        </>
                    )
                }
                {
                    aboutThisProperty?.bodySubSections.find(subSection =>
                        ...
                        <>
                            <h3 style={{ fontWeight: 'bold' }}>
                                {aboutThisProperty.bodySubSections.find(subSection => subSection?.elements[0]?.header?.text === "Languages ").elements[0].header.text}
                            </h3>
                            <p>
                                {aboutThisProperty.bodySubSections.find(subSection => subSection?.elements[0]?.header?.text === "Languages ").elements[0].items[0].content.primary.value}
                            </p>
                        </>
                    )
                }

...
const Profile = () => {
    ...
    const fetchSavedHotels = async () => {
        try {
            const responses = await Promise.all(user.savedHotels.map(hotel =>
                axios.get(`/api/hotelDetails/${hotel.hotelId}`).catch(err => {
                    console.error('Failed to fetch hotel details for hotelId:', hotel.hotelId, err);
                    return { data: { id: hotel.hotelId, name: hotel.hotelName, details: "Details not available" } }; // Fallback data structure
                })
            ));
    ...
    return (
        <div className="profile">
            <img src={user.profilePic || "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2c/Default_pfp.svg/2048px-Default_pfp.svg.png"} alt="Profile" className="profile-pic"/>
            ...
            <div className="profile-pic-update">
                <input type="text" placeholder="Enter new profile picture URL" value={newProfilePic} onChange={e => setNewProfilePic(e.target.value)} />
                <button onClick={updateProfilePicture}>Update Profile Picture</button>
            </div>
            <button onClick={toggleReviews} className="toggle-button">{showReviews ? 'Hide Reviews' : 'Show Reviews'}</button>
            {showReviews && (
                <div className="reviews">
                    {userReviews.map((review, index) => (
                        <div key={index} className="review-box">
                            <img src={review.hotelImageURL} alt="Hotel" className="hotel-image-small"/>
            ...
            <button onClick={toggleSavedHotels} className="toggle-button">{showSavedHotels ? 'Hide Saved Hotels' : 'Show Saved Hotels'}</button>
            {showSavedHotels && (
